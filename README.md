    ...
    def trap(self, height):
        ...
        for i in range(len(height)):
            ...
            while stack and height[i] > height[stack[-1]]:
                #바로 직전의 height의 인덱스를 꺼낸다. 변수 이름이 top이라 제일 높은 변곡점 얘기하는 줄 알고 코드 이해할 때 좀 헷갈렸는데, 스택의 꼭대기라는 의미에서 top이다.
                top = stack.pop()
                ...
                #물의 높이를 양 끝의 변곡점 중 낮은 변곡점과, 현 변곡점 바로 직전 칸의 높이를 이용하여 구한다. 근데 당최 정확히 어떤 원리로 물 높이가 딱 맞아지는 건지를 모르겠어서 이거 일주일지나서 구현해보라하면 못할 것 같다.
